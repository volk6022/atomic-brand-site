...
function makeApi(fixtures, misses) {
  return {
    ...
    // Экраны под этой проверкой не нажимают кнопки: прогон доводит их до
    // первой отрисовки. Но если ответ POST однажды доедет до renderVals
    // (сейчас так работает перечитывание после отправки черновика), заглушка
    // обязана повторять реальный ответ ручки — иначе ошибка формы не всплыла
    // бы нигде. Образцы берутся из того же дампа; нет образца — честный
    // {ok:true} по-старому.
    post: async (p) => {
      const key = fixtureKey(p);
      if (key in fixtures) return JSON.parse(JSON.stringify(fixtures[key]));
